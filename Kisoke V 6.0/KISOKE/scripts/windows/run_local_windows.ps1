...
function Start-FrontendIfNeeded {
  ...
  if ($Https) {
    Start-Process -FilePath "cmd.exe" -ArgumentList "/c set KISOKE_HTTPS=true&& set VITE_HTTPS=true&& npm run dev -- --host 0.0.0.0 --port $Port" -WorkingDirectory $ProjectRoot -WindowStyle Hidden
  } else {
    Start-Process -FilePath "cmd.exe" -ArgumentList "/c set KISOKE_HTTPS=&& set VITE_HTTPS=&& npm run dev -- --host 0.0.0.0 --port $Port" -WorkingDirectory $ProjectRoot -WindowStyle Hidden
  }
...
Write-Host ""
Write-Host "Project Nexora KISOKE is running"
Write-Host "================================"
