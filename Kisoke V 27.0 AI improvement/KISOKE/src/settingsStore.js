...
export const KISOKE_SETTINGS_KEY = 'kisoke.settings.v2';
export const KISOKE_SETTINGS_VERSION = 2;
const PRAYER_DUBAI_MIGRATION_KEY = 'kisoke.prayer-dubai-method-migration.v1';

const DEFAULTS = {
  version: KISOKE_SETTINGS_VERSION,
  appearance: {
    look: 'glossy',
    theme: 'auto',
...
export function loadKisokeSettings() {
  try {
    const stored = JSON.parse(localStorage.getItem(KISOKE_SETTINGS_KEY) || 'null');
    if (isRecord(stored)) {
      ...
      if (shouldMigratePrayerMethod) {
        ...
        return saveKisokeSettings(next);
      }
      return next;
    }
  } catch {
  ...
  saveKisokeSettings(migrated);
  return migrated;
}

export function saveKisokeSettings(value) {
  const next = merge(DEFAULTS, value);
  try {
    localStorage.setItem(KISOKE_SETTINGS_KEY, JSON.stringify(next));
  } catch {
...
export function resetKisokeSettingsSection(settings, section) {
  if (!Object.prototype.hasOwnProperty.call(DEFAULTS, section)) return settings;
  return saveKisokeSettings({ ...settings, [section]: clone(DEFAULTS[section]) });
}

export function exportKisokeSettings(settings) {
  return JSON.stringify(saveKisokeSettings(settings), null, 2);
}

export function importKisokeSettings(serialized) {
  const parsed = typeof serialized === 'string' ? JSON.parse(serialized) : serialized;
  if (!isRecord(parsed)) throw new Error('Settings file is not valid.');
  return saveKisokeSettings(parsed);
}

export function useKisokeSettings() {
  const [settings, setSettingsState] = useState(loadKisokeSettings);

  const updateSettings = useCallback((patch) => {
    setSettingsState((current) => {
      const nextValue = typeof patch === 'function' ? patch(current) : { ...current, ...patch };
      return saveKisokeSettings(nextValue);
    });
  }, []);
  ...
  const resetSection = useCallback((section) => {
    setSettingsState((current) => resetKisokeSettingsSection(current, section));
  }, []);

  return { settings, updateSettings, updateSection, resetSection };
}

export const KISOKE_DEFAULT_SETTINGS = DEFAULTS;
