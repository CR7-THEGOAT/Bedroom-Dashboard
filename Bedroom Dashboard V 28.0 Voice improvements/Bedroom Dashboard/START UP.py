#!/usr/bin/env python3
"""
KISOKE one-command startup for Windows and Ubuntu.
...
What it does:
  ...
  - downloads KISOKE AI models into ./ollama when possible
...
LOGS = ROOT / "logs"
HTTP_PORT = int(os.environ.get("KISOKE_HTTP_PORT", "5173"))
HTTPS_PORT = int(os.environ.get("KISOKE_HTTPS_PORT", "5174"))
BACKEND_PORT = int(os.environ.get("KISOKE_BACKEND_PORT", "8787"))
SKIP_MODELS = os.environ.get("KISOKE_SKIP_AI_MODELS", "").lower() in {"1", "true", "yes", "on"}
MODELS = [
    "gemma2:2b",
    "phi3.5",
    "llama3.2",
...
def download_ai_models(env: dict[str, str]) -> None:
    if SKIP_MODELS:
        print("KISOKE_SKIP_AI_MODELS is set, skipping model downloads.")
        return
...
def print_urls() -> None:
    ...
    print("Tip: set KISOKE_SKIP_AI_MODELS=true to skip model downloads next time.")
        ...
        while True:
            ...
            for process in processes:
                if process.poll() is not None:
                    print(f"Warning: a KISOKE process exited with code {process.returncode}. Check logs folder.")
                    processes.remove(process)
                    break
    except KeyboardInterrupt:
        print("\nStopping KISOKE processes started by this script...")
        return 0
