...
const ENABLE_HTTPS = /^(1|true|yes|on)$/i.test(process.env.KISOKE_HTTPS || process.env.VITE_HTTPS || '');
...
async function startBackendProcess(reason = 'FrontendAutostart', force = false) {
  ...
  if (process.platform === 'win32') {
    const scriptPath = path.join(projectRoot, 'scripts', 'windows', 'restart_kisoke_windows.ps1');
...
function getNetworkAccess(request) {
  ...
  return {
    ...
    hint: protocol === 'https'
      ...
      : 'HTTP LAN mode cannot use phone microphone/camera in Chrome. Start with KISOKE_HTTPS=true or use Tailscale HTTPS. If the page fails, Windows Firewall may be blocking ports 5173 or 8787.',
    urls,
    fetchedAt: Date.now()
  };
}
...
export default defineConfig({
  ...
  server: {
    proxy: {
      ...
      '/api/voice': { target: LOCAL_BACKEND, changeOrigin: true },
