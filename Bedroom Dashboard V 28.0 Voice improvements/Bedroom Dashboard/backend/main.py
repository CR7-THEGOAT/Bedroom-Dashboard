...
DEFAULT_BEDROOM_ESP32_URL = os.environ.get("BEDROOM_ESP32_URL", "http://192.168.4.51/")
DEFAULT_BEDROOM_WIFI_NAME = os.environ.get("BEDROOM_WIFI_NAME", "YOUR_WIFI_NAME")
DEFAULT_BEDROOM_ESP32_TIMEOUT = float(os.environ.get("BEDROOM_ESP32_TIMEOUT", "2.5"))
...
MUSIC_DIR = Path(os.environ.get("KISOKE_MUSIC_DIR", PROJECT_ROOT / "music"))
ALARM_SOUND_DIRS = [
    Path(os.environ.get("KISOKE_ALARM_SOUNDS_DIR", PROJECT_ROOT / "Custome Alarm Sounds")),
    PROJECT_ROOT / "Custom Alarm Sounds",
]
VIDEO_INTRO_DIRS = [
    Path(os.environ.get("KISOKE_VIDEO_INTROS_DIR", PROJECT_ROOT / "Custome Video Intro")),
...
def normalize_http_base_url(value: Any, default_url: str, label: str) -> tuple[str, str]:
    ...
    try:
        ...
        return "", f"Bad {label} URL: {error}"
    if parsed.scheme not in {"http", "https"}:
        return "", f"{label} URL must start with http:// or https://."
    if not parsed.hostname:
        return "", f"{label} URL needs an IP address or hostname."
    host = parsed.hostname.strip()
    if re.fullmatch(r"\d+\.\d+\.\d+\.\d+", host):
        octets = [int(part) for part in host.split(".")]
        if any(part < 0 or part > 255 for part in octets):
            return "", "That is not a valid IP address. Each part must be from 0 to 255."
    netloc = parsed.netloc
    if parsed.username or parsed.password:
        return "", f"{label} URL should not include a username or password."
    if port is not None and (port < 1 or port > 65535):
        return "", f"{label} URL port must be between 1 and 65535."
    return f"{parsed.scheme}://{netloc}/", ""


def normalize_bedroom_esp32_url(value: Any) -> tuple[str, str]:
    return normalize_http_base_url(value, DEFAULT_BEDROOM_ESP32_URL, "ESP32")


def normalize_ollama_url(value: Any) -> tuple[str, str]:
    return normalize_http_base_url(value, DEFAULT_OLLAMA_URL, "Ollama")


def ollama_settings() -> dict[str, Any]:
    settings = read_settings()
    url, error = normalize_ollama_url(settings.get("ollama_url") or DEFAULT_OLLAMA_URL)
    if error:
        url, _ = normalize_ollama_url(DEFAULT_OLLAMA_URL)
    return {"url": url or DEFAULT_OLLAMA_URL}


def ollama_url() -> str:
    return str(ollama_settings()["url"])


def clear_ollama_status_cache() -> None:
    OLLAMA_STATUS_CACHE["at"] = 0.0
    OLLAMA_STATUS_CACHE["data"] = None


def bedroom_urls_match(left: Any, right: Any) -> bool:
    left_text = str(left or "").strip()
    right_text = str(right or "").strip()
    if not left_text or not right_text:
        return False
    left_url, left_error = normalize_bedroom_esp32_url(left_text)
    right_url, right_error = normalize_bedroom_esp32_url(right_text)
    if left_error or right_error:
        return False
    return left_url.rstrip("/").lower() == right_url.rstrip("/").lower()


def bedroom_settings() -> dict[str, Any]:
    settings = read_settings()
    url, error = normalize_bedroom_esp32_url(settings.get("bedroom_esp32_url") or DEFAULT_BEDROOM_ESP32_URL)
    if error:
        url, _ = normalize_bedroom_esp32_url(DEFAULT_BEDROOM_ESP32_URL)
    timeout = clamp_number(settings.get("bedroom_esp32_timeout"), 0.5, 20.0, DEFAULT_BEDROOM_ESP32_TIMEOUT)
    return {
        "url": url or DEFAULT_BEDROOM_ESP32_URL,
        "wifi_name": str(settings.get("bedroom_wifi_name") or DEFAULT_BEDROOM_WIFI_NAME).strip() or DEFAULT_BEDROOM_WIFI_NAME,
        "timeout": timeout,
    }


def bedroom_timeout() -> float:
    return float(bedroom_settings()["timeout"])


def bedroom_join_url(base: str | None = None, path: str = "") -> str:
    clean_base = str(base or bedroom_settings()["url"]).rstrip("/")
...
def bedroom_json(path: str, timeout: float | None = None, base: str | None = None) -> dict[str, Any]:
    response = requests.get(
        ...
        timeout=timeout if timeout is not None else bedroom_timeout(),
...
def clear_bedroom_cache() -> None:
    with BEDROOM_CACHE_LOCK:
        BEDROOM_LAST_GOOD["at"] = 0.0
        BEDROOM_LAST_GOOD["data"] = {}
    try:
        if BEDROOM_CACHE_PATH.exists():
            BEDROOM_CACHE_PATH.unlink()
    except Exception:
        pass


def read_bedroom_cached(max_age: int = BEDROOM_CACHE_MAX_AGE, expected_base: str | None = None) -> tuple[float, dict[str, Any] | None, str]:
    now = time.time()

    def matches_expected(data: Any) -> bool:
        if not expected_base:
            return True
        if not isinstance(data, dict):
            return False
        return bedroom_urls_match(data.get("source_url"), expected_base)

    with BEDROOM_CACHE_LOCK:
        cached_at = float(BEDROOM_LAST_GOOD.get("at") or 0.0)
        cached_data = BEDROOM_LAST_GOOD.get("data")
    if isinstance(cached_data, dict) and cached_at and now - cached_at < max_age and matches_expected(cached_data):
    ...
    try:
        ...
        if isinstance(disk_data, dict) and disk_at and now - disk_at < max_age and matches_expected(disk_data):
            with BEDROOM_CACHE_LOCK:
...
def short_bedroom_error(error: Any) -> str:
    ...
    if "ConnectTimeout" in text or "timed out" in text:
        return f"ESP32 did not answer at {bedroom_base_url()} within {bedroom_timeout():g}s."
    ...
    if "No route to host" in text or "Network is unreachable" in text:
        return f"This laptop is not on the ESP32 network. Connect to {bedroom_settings()['wifi_name']}."
    if len(text) > 180:
        return f"{text[:177]}..."
    return text
...
def discover_bedroom_esp32(force: bool = False, base_url: str | None = None) -> str | None:
    ...
    with BEDROOM_CACHE_LOCK:
        discovered_at = float(BEDROOM_DISCOVERY.get("at") or 0.0)
        discovered_url = str(BEDROOM_DISCOVERY.get("url") or "").strip()
    if not force and discovered_url and time.time() - discovered_at < 30:
        return discovered_url

    parsed = urlparse(base_url or bedroom_settings()["url"])
...
def fetch_bedroom_esp32_data() -> dict[str, Any]:
    ...
    try:
        status = bedroom_json_retry("/api/status", attempts=2, timeout=bedroom_timeout(), base=active_base)
        try:
        ...
        if discovered_base and discovered_base.rstrip("/") != active_base.rstrip("/"):
            try:
                status = bedroom_json_retry("/api/status", attempts=1, timeout=bedroom_timeout(), base=discovered_base)
                try:
        ...
        cached_at, cached_data, cache_source = read_bedroom_cached(expected_base=active_base)
        ...
        response = requests.get(
            ...
            timeout=max(bedroom_timeout(), 4.5),
...
def proxy_bedroom_matrix(pixels: list[Any], pattern: str = "") -> dict[str, Any]:
    ...
    for method, path, kwargs in targets:
        ...
        try:
            if method == "GET":
                response = requests.get(url, timeout=bedroom_timeout())
            else:
                response = requests.post(url, timeout=bedroom_timeout(), **kwargs)
    ...
    for offset in range(0, total, chunk_size):
        ...
        try:
            response = requests.post(
                ...
                headers=headers,
                timeout=max(bedroom_timeout(), 8.0),
            )
...
def post_raw_octet_stream(url: str, payload: bytes) -> dict[str, Any]:
    ...
    sent = False
    try:
        with socket.create_connection((host, port), timeout=max(bedroom_timeout(), 8.0)) as sock:
            sock.settimeout(max(bedroom_timeout(), 8.0))
...
def proxy_bedroom_screen_image(filename: str, content_type: str, data_base64: str, slot: int = 1) -> dict[str, Any]:
    ...
    for path in targets:
        ...
        try:
            response = requests.post(
                url,
                timeout=max(bedroom_timeout(), 5.0),
...
def proxy_bedroom_screen_mode(mode: str, slot: int | None = None) -> dict[str, Any]:
    ...
    try:
        response = requests.get(bedroom_url(path), timeout=max(bedroom_timeout(), 4.0), allow_redirects=True)
...
DEFAULT_SETTINGS: dict[str, Any] = {
    ...
    "offline_voice_device": "auto",
    "offline_voice_sample_rate": 16000,
    "offline_voice_input_gain": 3.0,
    "offline_voice_target_rms": 1400.0,
    "offline_voice_noise_gate": 120.0,
    "offline_voice_min_confidence": 0.28,
    "offline_voice_debug": False,
    "offline_voice_wake_timeout": 10,
    ...
    "listen_timeout": 5,
    "bedroom_esp32_url": DEFAULT_BEDROOM_ESP32_URL,
    "bedroom_wifi_name": DEFAULT_BEDROOM_WIFI_NAME,
    "bedroom_esp32_timeout": DEFAULT_BEDROOM_ESP32_TIMEOUT,
...
class BedroomEsp32ConfigRequest(BaseModel):
    url: str | None = None
...
def ensure_camera_idle_watchdog() -> None:
    ...
    with CAMERA_IDLE_WATCHDOG_LOCK:
        ...
        threading.Thread(target=camera_idle_watchdog_loop, name="kisoke-camera-idle-watchdog", daemon=True).start()
...
def ollama_generate(model: str, prompt: str, timeout: int = 45) -> dict[str, Any]:
    ...
    try:
        ...
        response.raise_for_status()
        data = response.json()
        return {"ok": True, "model": model, "url": base_url, "reply": data.get("response", "").strip()}
        ...
        if "Connection" in type(error).__name__ or "connection" in str(error).lower():
            try:
                ...
                if start_result.get("ok"):
                    ...
                    response.raise_for_status()
                    data = response.json()
                    return {"ok": True, "model": model, "url": base_url, "reply": data.get("response", "").strip(), "autostart": start_result}
            except Exception:
                pass
        return {
            "ok": False,
...
def ollama_url_is_loopback(url: str) -> bool:
    try:
...
def ollama_status(timeout: float = 2.5, cache_ttl: float = 0.0) -> dict[str, Any]:
    ...
    if (
        ...
        and time.time() - float(OLLAMA_STATUS_CACHE.get("at") or 0) < cache_ttl
    ):
        return dict(cached)
    ...
    try:
        ...
        result = {
            "ok": True,
            "url": base_url,
            "local_url": ollama_url_is_loopback(base_url),
            "models": models,
...
def start_ollama_server() -> dict[str, Any]:
    ...
    if status.get("ok"):
        return {"ok": True, "message": "Ollama is already running.", "url": base_url, "models_dir": str(OLLAMA_MODEL_DIR)}
    if not ollama_url_is_loopback(base_url):
        return {
            "ok": False,
            "message": f"Ollama is set to {base_url}. Start Ollama on that device, or change the URL back to localhost.",
            "url": base_url,
            "models_dir": str(OLLAMA_MODEL_DIR),
            "remote": True,
        }
    ...
    if not executable:
        return {"ok": False, "message": "Ollama is not installed. Run the setup file or install Ollama first.", "url": base_url}
    ...
    try:
        ...
        return {"ok": False, "message": f"Could not start Ollama: {error}", "url": base_url, "models_dir": str(OLLAMA_MODEL_DIR)}
...
def launch_ai_model_download() -> dict[str, Any]:
    ...
    return {
        "ok": True,
        "message": f"AI model download started for {len(KISOKE_AI_MODELS)} models.",
        "models": KISOKE_AI_MODELS,
        "models_dir": str(OLLAMA_MODEL_DIR),
        "start": start_result,
    }
...
def build_command_classifier_prompt(command: str, settings: dict[str, Any], context: dict[str, Any] | None = None) -> str:
    ...
    return (
        f"You are {assistant_name}, the local KISOKE bedroom kiosk command router.\n"
...
def speak_in_background(text: str, rate: int | None, volume: float | None) -> dict[str, Any]:
    ...
    if rate is None:
        clean_rate = 0
    elif abs(float(rate)) > 20:
        clean_rate = int(round(clamp_number((float(rate) - 180) / 18, -10, 10, 0)))
    else:
        clean_rate = int(clamp_number(rate, -10, 10, 0))
    clean_volume = int(round(clamp_number(volume, 0, 1, 0.92) * 100)) if volume is not None else 92

    def speak_windows_sapi() -> bool:
        if not is_windows():
            return False
        powershell = shutil.which("powershell") or shutil.which("pwsh")
        if not powershell:
        ...
        command = (
            ...
            f"$speaker.Rate = {clean_rate}; "
            f"$speaker.Volume = {clean_volume}; "
            f"$speaker.Speak({json.dumps(clean_text)});"
        )
        try:
            completed = subprocess.run(
    ...
    def run() -> None:
        with TTS_LOCK:
            ...
            try:
                ...
                engine = pyttsx3.init()
                if rate is not None:
                    engine.setProperty("rate", max(90, min(260, int(rate))))
                ...
                try:
                    subprocess.run(["espeak", clean_text], check=False, timeout=20)
                except Exception:
                    pass

    threading.Thread(target=run, daemon=True).start()
    return {"ok": True, "message": "Speech request queued.", "engine": "system"}
...
def set_tailscale(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if enabled and payload.get("serve") is True:
        ...
        if not serve_result["ok"]:
            return serve_result
        return {**get_tailscale_status(), "message": "Tailscale VPN is on and serving KISOKE."}
...
def api_ai_config() -> dict[str, Any]:
    status = ollama_status(timeout=0.8, cache_ttl=10.0)
    return {
        "ok": True,
        "url": ollama_url(),
...
def api_ai_config_update(payload: OllamaConfigRequest, request: Request) -> dict[str, Any]:
    if not is_private_client(request):
        return {"ok": False, "error": "Ollama config is private network only."}
    url, error = normalize_ollama_url(payload.url)
    if error:
        return {"ok": False, "error": error, "url": payload.url}
    settings = patch_settings({"ollama_url": url})
    clear_ollama_status_cache()
    result: dict[str, Any] = {
        "ok": True,
        "url": settings["ollama_url"],
        "message": f"Ollama URL saved as {settings['ollama_url']}",
    }
    if payload.test:
        status = ollama_status(timeout=1.8, cache_ttl=0.0)
        result["test"] = status
        result["message"] = (
            f"Ollama URL saved and tested: {settings['ollama_url']}"
            if status.get("ok")
            else f"Ollama URL saved, but it did not answer: {status.get('error', 'waiting')}"
        )
    return result
...
def api_offline_voice_status() -> dict[str, Any]:
    ...
    if settings.get("offline_voice_enabled") and settings.get("offline_voice_autostart") and not service.status().get("running"):
...
@app.get("/api/voice/offline/test")
def api_offline_voice_test() -> dict[str, Any]:
    return offline_voice_service().self_test(read_settings())
...
def api_offline_voice_start(payload: OfflineVoiceRequest | None = None) -> dict[str, Any]:
    ...
    device_value = settings.get("offline_voice_device", settings.get("offlineVoiceDevice", "auto"))
    device_value = str(device_value).strip() if device_value is not None else "auto"
    patch_settings({
        "offline_voice_enabled": True,
        "offline_voice_autostart": bool(settings.get("offline_voice_autostart", settings.get("offlineVoiceAutostart", True))),
        ...
        "offline_voice_device": device_value or "auto",
        "offline_voice_sample_rate": settings.get("offline_voice_sample_rate", settings.get("offlineVoiceSampleRate", 16000)),
        "offline_voice_input_gain": settings.get("offline_voice_input_gain", settings.get("offlineVoiceInputGain", 3.0)),
        "offline_voice_target_rms": settings.get("offline_voice_target_rms", settings.get("offlineVoiceTargetRms", 1400.0)),
        "offline_voice_noise_gate": settings.get("offline_voice_noise_gate", settings.get("offlineVoiceNoiseGate", 120.0)),
        "offline_voice_min_confidence": settings.get("offline_voice_min_confidence", settings.get("offlineVoiceMinConfidence", 0.28)),
        "offline_voice_debug": settings.get("offline_voice_debug", settings.get("offlineVoiceDebug", settings.get("voiceDebug", False))),
        "offline_voice_wake_timeout": settings.get("offline_voice_wake_timeout", settings.get("offlineVoiceWakeTimeout", 10)),
...
def api_offline_voice_settings(payload: OfflineVoiceRequest) -> dict[str, Any]:
    ...
    device_value = request_settings.get("offline_voice_device", request_settings.get("offlineVoiceDevice", current_settings.get("offline_voice_device", "auto")))
    device_value = str(device_value).strip() if device_value is not None else "auto"
    patch = {
        ...
        "offline_voice_input_gain": request_settings.get("offline_voice_input_gain", request_settings.get("offlineVoiceInputGain", current_settings.get("offline_voice_input_gain", 3.0))),
        "offline_voice_target_rms": request_settings.get("offline_voice_target_rms", request_settings.get("offlineVoiceTargetRms", current_settings.get("offline_voice_target_rms", 1400.0))),
        "offline_voice_noise_gate": request_settings.get("offline_voice_noise_gate", request_settings.get("offlineVoiceNoiseGate", current_settings.get("offline_voice_noise_gate", 120.0))),
        "offline_voice_min_confidence": request_settings.get("offline_voice_min_confidence", request_settings.get("offlineVoiceMinConfidence", current_settings.get("offline_voice_min_confidence", 0.28))),
        "offline_voice_debug": bool(request_settings.get("offline_voice_debug", request_settings.get("offlineVoiceDebug", request_settings.get("voiceDebug", current_settings.get("offline_voice_debug", False))))),
        "offline_voice_wake_timeout": request_settings.get("offline_voice_wake_timeout", request_settings.get("offlineVoiceWakeTimeout", 10)),
...
@app.get("/api/bedroom/config")
def api_bedroom_config() -> dict[str, Any]:
    config = bedroom_settings()
    with BEDROOM_CACHE_LOCK:
        discovered_url = str(BEDROOM_DISCOVERY.get("url") or "").strip()
        discovered_at = float(BEDROOM_DISCOVERY.get("at") or 0.0)
    return {
        ...
        "discovered_age_seconds": round(time.time() - discovered_at) if discovered_url and discovered_at else None,
    }


@app.post("/api/bedroom/config")
def api_bedroom_config_update(payload: BedroomEsp32ConfigRequest, request: Request) -> dict[str, Any]:
    if not is_private_client(request):
        return {"ok": False, "error": "Bedroom ESP32 config is private network only."}
    url, error = normalize_bedroom_esp32_url(payload.url)
    if error:
        return {"ok": False, "error": error, "url": payload.url}
    timeout = clamp_number(payload.timeout, 0.5, 20.0, DEFAULT_BEDROOM_ESP32_TIMEOUT)
    wifi_name = str(payload.wifi_name or DEFAULT_BEDROOM_WIFI_NAME).strip() or DEFAULT_BEDROOM_WIFI_NAME
    settings = patch_settings({
        "bedroom_esp32_url": url,
        "bedroom_wifi_name": wifi_name,
        "bedroom_esp32_timeout": timeout,
    })
    clear_bedroom_cache()
    with BEDROOM_CACHE_LOCK:
        BEDROOM_DISCOVERY["at"] = 0.0
        BEDROOM_DISCOVERY["url"] = ""
    result: dict[str, Any] = {
        "ok": True,
        "url": settings["bedroom_esp32_url"],
        "wifi_name": settings["bedroom_wifi_name"],
        "timeout": settings["bedroom_esp32_timeout"],
        "message": f"ESP32 address saved as {settings['bedroom_esp32_url']}",
    }
    if payload.test:
        try:
            status = bedroom_json("/api/status", timeout=timeout, base=url)
            result["test"] = {"ok": True, "status": status}
            result["message"] = f"ESP32 address saved and tested: {url}"
        except Exception as test_error:
            result["test"] = {"ok": False, "error": short_bedroom_error(test_error), "error_detail": str(test_error)}
            result["message"] = f"ESP32 address saved, but the test did not connect: {short_bedroom_error(test_error)}"
    return result


@app.post("/api/bedroom/discover")
def api_bedroom_discover(request: Request) -> dict[str, Any]:
    if not is_private_client(request):
        return {"ok": False, "error": "Bedroom ESP32 discovery is private network only."}
    found = discover_bedroom_esp32(force=True, base_url=bedroom_settings()["url"])
    if not found:
        return {"ok": False, "error": "Could not find an ESP32 Smart Hub on the configured subnet."}
    settings = patch_settings({"bedroom_esp32_url": found})
    clear_bedroom_cache()
    return {
        "ok": True,
        "url": settings["bedroom_esp32_url"],
        "wifi_name": settings.get("bedroom_wifi_name") or DEFAULT_BEDROOM_WIFI_NAME,
        "timeout": settings.get("bedroom_esp32_timeout") or DEFAULT_BEDROOM_ESP32_TIMEOUT,
        "message": f"Found and saved ESP32 at {found}",
    }
...
def api_bedroom_matrix(payload: dict[str, Any], request: Request) -> dict[str, Any]:
    ...
    return {
        **result,
        "source": bedroom_base_url(),
        "wifi_name": bedroom_settings()["wifi_name"],
        "fetched_at": time.time(),
    }
...
def open_camera_capture(cv2: Any, device: int):
    ...
    if is_windows():
        ...
        if _env_truthy(os.environ.get("KISOKE_CAMERA_ALLOW_DSHOW")):
