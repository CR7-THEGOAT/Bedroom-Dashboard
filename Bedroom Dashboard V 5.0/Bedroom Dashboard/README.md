...
cat > code-needed-to-download.sh <<'BEDROOM_DASHBOARD_INSTALL'
#!/usr/bin/env bash
set -euo pipefail
...
- On Windows it runs `restart_BEDROOM_DASHBOARD_windows.ps1 -BackendOnly`; on Ubuntu it starts the Python backend directly.
- This requires running Bedroom Dashboard through `run_local_windows.ps1`, `start-kiosk.sh`, or `npm run dev`. A static `dist` folder alone cannot start Python.
