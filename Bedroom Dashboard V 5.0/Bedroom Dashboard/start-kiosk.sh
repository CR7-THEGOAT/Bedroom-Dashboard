...
HTTP_PORT="${BEDROOM_DASHBOARD_HTTP_PORT:-5173}"
HTTPS_PORT="${BEDROOM_DASHBOARD_HTTPS_PORT:-5174}"
...
if [ -f "backend/main.py" ]; then
  ...
  if ! command -v ss >/dev/null 2>&1 || ! ss -ltn 2>/dev/null | grep -q ':8787 '; then
    ALLOW_DEVICE_CONTROL="${ALLOW_DEVICE_CONTROL:-true}" \
      ...
      > backend-linux.out.log 2> backend-linux.err.log &
    BACKEND_PID=$!
  fi
fi
...
BEDROOM_DASHBOARD_HTTPS= VITE_HTTPS= npm run dev -- --host 0.0.0.0 --port "$HTTP_PORT" \
  > vite-http-linux.out.log 2> vite-http-linux.err.log &
HTTP_FRONTEND_PID=$!

BEDROOM_DASHBOARD_HTTPS=true VITE_HTTPS=true npm run dev -- --host 0.0.0.0 --port "$HTTPS_PORT" \
  > vite-https-linux.out.log 2> vite-https-linux.err.log &
HTTPS_FRONTEND_PID=$!
...
BROWSER_URL="${BEDROOM_DASHBOARD_KIOSK_URL:-http://localhost:$HTTP_PORT/}"

echo "Bedroom Dashboard HTTP:  http://localhost:$HTTP_PORT"
echo "Bedroom Dashboard HTTPS: https://localhost:$HTTPS_PORT"
