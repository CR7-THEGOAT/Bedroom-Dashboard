...
KIOSK_DIR="$HOME/Documents/Bedroom Dashboard"
...
Exec=bash -lc 'cd "$KIOSK_DIR" && ALLOW_DEVICE_CONTROL=true bash start-kiosk.sh'
...
ExecStart=/usr/bin/env bash start-kiosk.sh
Restart=always
RestartSec=8
