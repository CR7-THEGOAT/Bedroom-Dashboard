...
class CameraSensorManager:
    ...
    def switch_camera_background(self, camera_index: int) -> None:
        ...
        threading.Thread(target=worker, name="bedroom-dashboard-camera-switch", daemon=True).start()
    ...
    def start(self, camera_index: int = 0) -> None:
        ...
        with self._lock:
            ...
            self._thread = threading.Thread(target=self._loop, args=(generation,), name="bedroom-dashboard-local-camera-sensor", daemon=True)
            self._thread.start()
    ...
    def _open_capture(self, cv2: Any, camera_index: int):
        ...
        if platform.system() == "Windows":
            ...
            if str(os.environ.get("BEDROOM_DASHBOARD_CAMERA_ALLOW_DSHOW", "")).lower() in {"1", "true", "yes", "on"}:
