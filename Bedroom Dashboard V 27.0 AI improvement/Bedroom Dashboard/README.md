...
Offline voice:  http://localhost:8787/api/voice/offline/status
...
This is the single Ubuntu code block. Run it from the Bedroom Dashboard folder:

```bash
cd ~/Documents/Bedroom Dashboard

mkdir -p scripts/install
cat > scripts/install/code-needed-to-download.sh <<'BEDROOM_DASHBOARD_INSTALL'
#!/usr/bin/env bash
set -euo pipefail

echo "Installing Bedroom Dashboard software..."
...
- `Bedroom Dashboard Autostart` starts Bedroom Dashboard when you log in after shutdown/restart.
- `Bedroom Dashboard Wake Recover` restarts Bedroom Dashboard after the laptop wakes from sleep when Windows exposes the wake event.
- `Bedroom Dashboard Watchdog` checks every 2 minutes and starts the backend/frontend if either one is down.
...
```text
C:\Users\saeed\AppData\Roaming\Microsoft\Windows\Start Menu\Programs\Startup\Bedroom-Dashboard-Watchdog.vbs
```

That fallback starts after login and checks Bedroom Dashboard every 30 seconds. After sleep, it resumes with Windows and restarts the backend/frontend if they died during sleep.
...
- On Windows it runs `scripts/windows/restart_BEDROOM_DASHBOARD_windows.ps1 -BackendOnly`; on Ubuntu it starts the Python backend directly.
- This requires running Bedroom Dashboard through `scripts/windows/run_local_windows.ps1`, `scripts/linux/start-kiosk.sh`, `START UP.py`, or `npm run dev`. A static `dist` folder alone cannot start Python.
...
```text
C:\Users\saeed\OneDrive\Documents\Bedroom Dashboard\logs\auto-restart.log
```
...
```powershell
$env:BEDROOM_DASHBOARD_HTTP="true"
powershell -ExecutionPolicy Bypass -File .\scripts\windows\run_local_windows.ps1
```
...
```powershell
$env:BEDROOM_DASHBOARD_SKIP_AI_MODELS="true"
py "START UP.py"
```
...
```bash
BEDROOM_DASHBOARD_SKIP_AI_MODELS=true python3 "START UP.py"
```

### Bedroom Dashboard Browser Shell

Bedroom Dashboard now includes an Electron/Chromium shell under `electron/main.cjs`. It opens the local dashboard without the normal Chrome toolbar or menus.
...
- `Ctrl + R`: reload Bedroom Dashboard
- `Ctrl + Shift + Q`: exit the desktop shell

The shell uses the existing Vite frontend in development. The normal `npm run build` command still creates the production frontend files in `dist`.
