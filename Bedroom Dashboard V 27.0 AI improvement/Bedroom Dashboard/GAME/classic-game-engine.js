(function () {
  const cfg = window.BEDROOM_DASHBOARD_GAME || {};
  const root = document.getElementById('game-root');
  const modeLabels = { ai: 'AI vs Player 1', two: '2 Player' };
  const state = {
    mode: localStorage.getItem('Bedroom Dashboard.game.mode') || 'ai',
    scores: [0, 0],
    running: false,
    cleanup: null
  };
  ...
  function shell() {
    const title = cfg.title || 'Bedroom Dashboard Game';
        ...
        <main class="game-main">
          ...
          <aside class="side-panel">
            ...
            <section class="game-card">
              ...
              <p>Runs offline from the Bedroom Dashboard GAME folder.</p>
            </section>
          </aside>
        </main>
      </section>
    ...
    document.querySelectorAll('[data-mode]').forEach((button) => {
      button.addEventListener('click', () => {
        ...
        localStorage.setItem('Bedroom Dashboard.game.mode', state.mode);
        boot();
      });
    });
