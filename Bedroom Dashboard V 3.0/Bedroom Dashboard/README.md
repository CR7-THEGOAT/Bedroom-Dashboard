...
Bedroom Dashboard is a local smart bedroom kiosk with a clock page, dashboard page, settings page, local command assistant, browser voice input, camera brightness theme switching, and an optional local FastAPI backend for Ollama.
...
The app also has a built-in **Software Needed** page in Settings. Open Bedroom Dashboard, go to Settings, then tap **Software needed** to see the same checklist with copyable commands and backend status.
...
cd ~/Documents/Bedroom Dashboard/backend
python3 -m venv .venv
...
export OLLAMA_MODELS="$HOME/Documents/Bedroom Dashboard/ollama"
...
Easy model: `qwen2.5:3b`

Hard model: `qwen3:4b`

Small backup model: `llama3.2:1b`

On Windows, this project is using `C:\Users\saeed\OneDrive\Documents\Bedroom Dashboard\ollama` as the Ollama model folder.

Windows PowerShell model-folder setup:

```powershell
[Environment]::SetEnvironmentVariable('OLLAMA_MODELS', 'C:\Users\saeed\OneDrive\Documents\Bedroom Dashboard\ollama', 'User')
$env:OLLAMA_MODELS='C:\Users\saeed\OneDrive\Documents\Bedroom Dashboard\ollama'
ollama pull llama3.2:1b
ollama pull qwen2.5:3b
ollama pull qwen3:4b
```
...
- Time Deck clock page with vertical scroll sections for Live Clock, Alarm, Countdown, Stopwatch, World Clock, Prayer Focus, and Sleep Clock.
- Settings Core Console with health check, notification previews, weather debug, and software-needed page at `/software-needed`.
...
- Browser speech recognition when the browser supports it. Tap the mic once to arm Chrome permission; after that always-listening runs while the Bedroom Dashboard tab is open and awake.
