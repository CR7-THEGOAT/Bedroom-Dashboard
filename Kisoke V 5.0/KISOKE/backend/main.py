...
def api_local_camera_restart(payload: dict[str, Any], request: Request) -> dict[str, Any]:
    ...
    camera_index = max(0, min(8, camera_index))
...
def api_local_camera_mode(payload: dict[str, Any], request: Request) -> dict[str, Any]:
    ...
    settings = read_settings()
    camera_index = int(payload.get("camera_index", settings.get("camera_device", 0)) or 0)
    patch = {"usb_camera_mode": mode, "camera_device": max(0, min(8, camera_index))}
...
def api_remote_camera_settings(payload: dict[str, Any], request: Request) -> dict[str, Any]:
    ...
    if "camera_device" in payload:
        try:
            patch["camera_device"] = max(0, min(8, int(payload.get("camera_device"))))
        except (TypeError, ValueError):
            pass
...
def list_camera_devices(max_devices: int = 4, force_refresh: bool = False) -> dict[str, Any]:
    cache_ttl = 30.0
    ...
    try:
        ...
        return {
            "ok": False,
            "error": f"OpenCV is not installed: {error}",
            "devices": [{"id": index, "label": f"Device {index}", "available": False} for index in range(max_devices)],
        }
    ...
    with CAMERA_LOCK:
        for index in range(max_devices):
            if active_sensor_index == index:
                devices.append({"id": index, "label": camera_label(index, " (sensor active)"), "available": True})
                continue
            if REMOTE_CAMERA_CAPTURE is not None and REMOTE_CAMERA_CAPTURE_DEVICE == index and safe_capture_is_open(REMOTE_CAMERA_CAPTURE):
                devices.append({"id": index, "label": camera_label(index, " (active)"), "available": True})
                continue
            cap = open_camera_capture(cv2, index)
            try:
                available = safe_capture_is_open(cap)
                if available:
                    safe_capture_set(cap, cv2.CAP_PROP_FRAME_WIDTH, 160)
                    safe_capture_set(cap, cv2.CAP_PROP_FRAME_HEIGHT, 120)
                devices.append({
                    "id": index,
...
def api_remote_camera_stream(request: Request, token: str = "", camera: int | None = None):
    ...
    if camera is not None:
        try:
            selected_camera = max(0, min(8, int(camera)))
            settings = read_settings()
